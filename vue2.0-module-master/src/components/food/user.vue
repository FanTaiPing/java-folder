  <template>
    ...
    <el-table :data="userList" highlight-current-row v-loading="loading" element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column prop="nick_name" label="用户名称" width="300" >
      </el-table-column>
      <el-table-column prop="name" label="昵称" width="200" >
      </el-table-column>
      <el-table-column prop="tzyx" label="投资意向" width="300" >
      </el-table-column>
...
-->
    ...
    <el-dialog title="处理" v-model="editFormVisible">
      <el-form label-width="80px" :model="editForm" ref="editForm">
        ...
        <el-form-item label="投资意向">
          <!-- {{editForm.tzyx}} -->
          <el-input v-model="editForm.tzyx"></el-input>
        </el-form-item>

        <el-form-item label="申请时间">
          <!-- {{editForm.save_datetime}} -->
          <el-input v-model="editForm.save_datetime"></el-input>
        </el-form-item>

        <el-form-item label="处理意见">
          <el-input type="textarea" row="10" v-model="editForm.clyj"></el-input>
        </el-form-item>
   ...
   data() {
    ...
    methods: {
      ...
      query(){
        ...
        this.$http.get('api/user/findAllMap?name='+this.paramQuery.name).then((response) => {
      ...
      getuserData(callback){
        this.loading = true
        this.$http.get('/api/user/findAllMap').then((response) => {
