            ...
            <div class="col-10 border rounded-3 bg-light">
                <div id="author-two">
                    <table class="form-show">
                        <tr>
                            <td style="text-align: left;">作者信息显示界面</td>
                            <td>

                            </td>
                            <td style="width: 430px;text-align: center">
                                <a class="btn btn-primary" style="margin-right: 15px;" href="javascript:window.location='${pageContext.request.contextPath}/pages/addAuthor.jsp'">添加作者</a>
                                <a class="btn btn-primary" href="javascript:void(0);" id="delSelected">删除选中</a>
                            </td>
                        </tr>
                    </table>
                    <form action="${pageContext.request.contextPath }/DeleteAuthorByIdServlet" method="post" id="form">
                    <table class="table table-bordered" id="myTable">
                        ...
                        <c:forEach items="${requestScope.pageBeanAuthor.list}" var="pageBeanAuthor">
                            <tr>
                                <td><input type ="checkbox" name="uid" value="${pageBeanAuthor.id }"></td>
                                <td>${pageBeanAuthor.id}</td>
                                <td>${pageBeanAuthor.name}</td>

                                <td>${pageBeanAuthor.dynasty}</td>
                                <td>${pageBeanAuthor.lifeTime}</td>
                                <td>${pageBeanAuthor.pinyin}</td>
                                <td>
                                    <a class="btn btn-info btn_btn" href="${pageContext.request.contextPath}/FindAuthorByIdServlet?id=${pageBeanAuthor.id}">修改信息</a>
                                    <a class="btn btn-info btn_btn" href="javascript:deletePoetry(${pageBeanAuthor.id})">删除信息</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
                    ...
                    <script>
                        function deletePoetry(id) {
                            ...
                            if (confirm("您确定要删除吗？")) {
                                //访问路径
                                location.href = "${pageContext.request.contextPath}/DeleteAuthorByIdServlet?id=" + id;
                            }
                        }
                    </script>
                    <!-- 分页部分 -->
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-lg">
                            ...
                            <c:if test="${requestScope.pageBeanAuthor.currentPage == 1 }">
                                <li class="page-item disabled">
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath }/AuthorServlet?currentPage=${requestScope.pageBeanAuthor.currentPage-1}&rows=10"
                                       aria-label="Previous">
                            ...
                            <c:if test="${requestScope.pageBeanAuthor.currentPage != 1 }">
                                ...
                                <li>
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath }/AuthorServlet?currentPage=1&rows=5"
                                       aria-label="Previous">
                                        <span aria-hidden="true">首页</span>
                                    </a>
                                </li>
                            <%-- 从1开始循环，总的有几页就循环几次，显示页码数字 --%>
                            <c:forEach begin="${requestScope.pageBeanAuthor.currentPage}"
                                       end="${requestScope.pageBeanAuthor.currentPage+8 }" var="i">
                                <c:if test="${requestScope.pageBeanAuthor.currentPage == i }">
                                    <li class="page-item active">
                                        <a class="page-link"
                                           href="${pageContext.request.contextPath }/AuthorServlet?currentPage=${i}&rows=10">${i}</a>
                                    </li>
                                </c:if>
                                <c:if test="${requestScope.pageBeanAuthor.currentPage != i }">
                                    <li class="page-item">
                            ...
                            <c:if test="${requestScope.pageBeanAuthor.currentPage >= requestScope.pageBeanAuthor.totalPage }">
                                <li class="disabled">
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath }/AuthorServlet?currentPage=${requestScope.pageBeanAuthor.currentPage}&rows=10"
                                       aria-label="Previous">
                            ...
                            <li class="page-item"><span class="totalCount">共${requestScope.pageBeanAuthor.totalCount }条记录，共${requestScope.pageBeanAuthor.totalPage }页</span>
                            </li>
                        </ul>
                    </nav>
                </div>
