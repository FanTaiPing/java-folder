            ...
            <div class="col-10 border rounded-3 bg-light">
                <div id="type-four">
                    <table class="form-show">
                        <tr>
                            <td style="text-align: left;">诗词类型信息显示界面</td>
                            <td>

                            </td>
                            <td style="width: 430px;text-align: center">
                                <a class="btn btn-primary" style="margin-right: 15px;" href="javascript:window.location='${pageContext.request.contextPath}/pages/addType.jsp'">添加诗词类型</a>
                                <a class="btn btn-primary" href="javascript:void(0);" id="delSelected">删除选中</a>
                            </td>
                        </tr>
                    </table>
                    <form action="${pageContext.request.contextPath }/DeleteTypeByIdServlet" method="post">
                    <table class="table table-bordered">
                        ...
                        <c:forEach items="${requestScope.pageBeanType.list}" var="pageBeanType">
                            <tr>
                                <td><input type ="checkbox" name="uid" value="${pageBeanType.id }"></td>
                                <td>${pageBeanType.id}</td>
                                <td>${pageBeanType.typeName}</td>
                                <td>
                                    <a class="btn btn-info btn_btn" href="${pageContext.request.contextPath}/FindTypeByIdServlet?id=${pageBeanType.id}">修改信息</a>
                                    <a class="btn btn-info btn_btn" href="javascript:deletePoetry(${pageBeanType.id})">删除信息</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
                    ...
                    <script>
                        function deletePoetry(id) {
                            ...
                            if (confirm("您确定要删除吗？")) {
                                //访问路径
                                location.href = "${pageContext.request.contextPath}/DeleteTypeByIdServlet?id=" + id;
                            }
                        }
                    </script>
                    <%--分页部分--%>
                    <nav aria-label="Page navigation example">
                        <ul class="pagination pagination-lg">
                            ...
                            <c:if test="${requestScope.pageBeanType.currentPage == 1 }">
                                <li class="page-item disabled">
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath }/TypeServlet?currentPage=${requestScope.pageBeanType.currentPage - 1}&rows=10"
                                       aria-label="Previous">
                            ...
                            <c:if test="${requestScope.pageBeanType.currentPage != 1 }">
                                ...
                                <li>
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath }/TypeServlet?currentPage=1&rows=5"
                                       aria-label="Previous">
                                        <span aria-hidden="true">首页</span>
                                    </a>
                                </li>
                            <%--从1开始循环，总的有几页就循环几次，显示页码数字--%>
                            <c:forEach begin="2" end="${requestScope.pageBeanType.totalPage }" var="i">
                                <c:if test="${requestScope.pageBeanType.currentPage == i }">
                                    <li class="page-item active">
                                        <a class="page-link"
                                           href="${pageContext.request.contextPath }/TypeServlet?currentPage=${i}&rows=10">${i}</a>
                                    </li>
                                </c:if>
                                <c:if test="${requestScope.pageBeanType.currentPage != i }">
                                    <li class="page-item">
                            ...
                            <c:if test="${requestScope.pageBeanType.currentPage >= requestScope.pageBeanType.totalPage }">
                                <li class="disabled">
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath }/TypeServlet?currentPage=${requestScope.pageBeanType.currentPage}&rows=10"
                                       aria-label="Previous">
                            ...
                            <li class="page-item"><span class="totalCount">共${requestScope.pageBeanType.totalCount }条记录，共${requestScope.pageBeanType.totalPage }页</span>
                            </li>
                        </ul>
                    </nav>
                </div>
